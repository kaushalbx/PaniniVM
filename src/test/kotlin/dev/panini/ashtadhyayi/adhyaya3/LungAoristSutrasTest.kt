...
class LungAoristSutrasTest {
    ...
    fun `test 3 1 48 NishriDruSruBhyahKarthariChaSutra`() {
        ...
        assertTrue(NishriDruSruBhyahKarthariChaSutra.matches(state))
        assertTrue(NishriDruSruBhyahKarthariChaSutra.apply(state).state.allEffectiveTerms.any { it.upadesha == "चङ्" })
    }
    ...
    fun `test 3 1 55 PusAdiDyutAdyLdtahParasmaipadesuSutra`() {
        ...
        assertTrue(PusAdiDyutAdyLdtahParasmaipadesuSutra.matches(state))
        assertTrue(PusAdiDyutAdyLdtahParasmaipadesuSutra.apply(state).state.allEffectiveTerms.any { it.upadesha == "अङ्" })
    }
    ...
    fun `test 3 1 60 ChinKarmaniChaSutra`() {
        ...
        assertTrue(ChinKarmaniChaSutra.matches(state))
        assertTrue(ChinKarmaniChaSutra.apply(state).state.allEffectiveTerms.any { it.upadesha == "चिण्" })
    }
}
