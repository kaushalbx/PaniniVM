...
class TaparasTatKalasyaRuntimeTest {
    @Test
    fun `same-duration principle round trips through canonical blueprint`() {
        val blueprint = TaparasTatKalasyaSutra.toBlueprint()
    ...
    @Test
    fun `decoded same-duration principle registers natively`() {
        val blueprint = TaparasTatKalasyaSutra.toBlueprint()
        val encoded = assertIs<SutraBlueprintTextEncoding.Success>(
            SutraBlueprintTextCodec.encode(blueprint),
        ).text
        ...
        val initial = DerivationAvastha(
            DerivationState(
                terms = listOf(
                    DerivationTerm("varna", "अत्", TermKind.PRATIPADIKA),
                ),
            ),
        )
        ...
        val result = assertIs<SutraMachineResult.Success<DerivationAvastha>>(
            SutraMachine(DerivationSutraEffectInterpreter).process(
                SutraProgram("ashtadhyayi-1.1.70", listOf(runtime)),
                initial,
            ),
        )
        ...
        assertEquals(
            setOf(
                InterpretivePrincipleDefinition(
                    principle = InterpretivePrinciple.T_MARKED_SOUND_SAME_DURATION,
                    definingSutra = SutraId("1.1.70"),
                ),
            ),
            result.state.interpretivePrinciples,
        )
        assertEquals(listOf(SutraId("1.1.70")), result.state.appliedSutras)
    }

    @Test
    fun `ashtadhyayi grantha selects native same-duration principle automatically`() {
        val runtime = AshtadhyayiRuntimeGrantha.grantha.sutras.single {
            it.id == SutraId("1.1.70")
        }
    ...
    fun `legacy direct behavior remains available during migration`() {
        assertEquals(true, TaparasTatKalasyaSutra.apply("अत्"))
    }
}
