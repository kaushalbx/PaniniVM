...
import dev.panini.ashtadhyayi.adhyaya3.pada1.svaraVowelPositions
...
object AnudattamPadamEkavarjamSutra : Sutra<DerivationState, DerivationChange>(
    ...
    override fun matches(context: DerivationState): Boolean =
        context.svaraAssignments.any { it.accent == AccentType.UDATTA } &&
            context.svaraAssignments.map { it.vowelIndex }.distinct().size < svaraVowelPositions(context.surface).size

    override fun apply(context: DerivationState): DerivationChange {
        val udatta = context.svaraAssignments.single { it.accent == AccentType.UDATTA }.vowelIndex
        val additions = svaraVowelPositions(context.surface).indices.filter { it != udatta }
